...
import Exercise2b from "./Exercise2b";

const Exercise2a = () => {
  const [visit, setVisit] = useState(0);

  useEffect(() => {
    console.log("Timer has re-rendered");
  }, [visit]);
  useEffect(() => {
    return console.log("Timer has rendered");
  }, []);
  return (
    <div>
      <h1>Visits : {visit}</h1>
      <button onClick={() => setVisit((prev) => prev + 1)}>Add visit</button>
      <Exercise2b />
    </div>
  );
};
